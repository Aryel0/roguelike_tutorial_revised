...
from actions import Action, BumpAction, PickupAction, WaitAction, ThrowProjectileAction
import actions
import color
import exceptions
...
class CharacterScreenEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        console.print(x=x + 1, y=y + 3, string=f"XP for next: {player.level.experience_to_next_level}")

        console.print(x=x + 1, y=y + 5, string="=== Stats ===", fg=(255, 255, 100))
        console.print(x=x + 1, y=y + 6, string=f"Attack: {player.fighter.power} ({player.fighter.base_power}+{player.fighter.power_bonus})")
        console.print(x=x + 1, y=y + 7, string=f"Defense: {player.fighter.defense} ({player.fighter.base_defense}+{player.fighter.defense_bonus})")

        console.print(x=x + 1, y=y + 9, string="=== Equipment ===", fg=(100, 255, 100))
        ...
        console.print(x=x + 1, y=y + 10, string=f"Weapon: {weapon_name}")
        console.print(x=x + 1, y=y + 11, string=f"Armor: {armor_name}")
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        """
        super().on_render(console)
        inventory_items = self.engine.player.inventory.items
        number_of_items = len(inventory_items)
        ...
        if number_of_items > 0:
            for i, entry in enumerate(inventory_items):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if 0 <= index <= 26:
            try:
                # Use get_item to unpack the tuple safely
...
class MainGameEventHandler(EventHandler):
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if key == tcod.event.K_d and modifier & (tcod.event.KMOD_LSHIFT | tcod.event.KMOD_RSHIFT):
            return actions.TakeStairsAction(player)

        if key in MOVE_KEYS:
            dx, dy = MOVE_KEYS[key]
            self.engine.player.facing_direction = (dx, dy)
            ...
            dx, dy = self.engine.player.facing_direction
            return ThrowProjectileAction(self.engine.player,dx,dy)
